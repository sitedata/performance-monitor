...
class Indexer {
  ...
  public static function writeLog( $msg, $maxlength = 256, $name = 'log' ) {
    ...
    $log      = get_transient( PERFORMANCE_MONITOR_PREFIX . $name );
    ...
    set_transient( PERFORMANCE_MONITOR_PREFIX . $name, $log, PERFORMANCE_MONITOR_LONG_LIFETIME );
  ...
  public function fake_views_users( $views ) {
    ...
    $replacement = esc_attr__( 'Still counting users...', 'performance-monitor' );
  ...
  public function removeIndexRole( $user_id, $blog_id ) {
    ...
    $prefix = $wpdb->prefix . PERFORMANCE_MONITOR_KEY_PREFIX . 'r:';
  ...
  public function updateIndexRole( $user_id, $role, $blog_id ) {
    ...
    try {
      ...
      $wpdb->query( 'ROLLBACK' );
      error_log( 'performance_monitor: updateIndexRole exception: ' . $ex->getMessage() . ' ' . $ex->getTraceAsString() );
    } finally {
      if ( is_multisite() ) {
        restore_current_blog();
      }
