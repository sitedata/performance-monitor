 ...
 */
function performance_monitor_do_task( $taskName, $step ) {
  $task = null;
  try {
    ...
    error_log( 'performance_monitor_task: cron hook exception: ' . $taskName . ' ' . $ex->getMessage() . ' ' . $ex->getTraceAsString() );
  }
}

add_action( 'performance_monitor_task', __NAMESPACE__ . '\performance_monitor_do_task', 10, 2 );
add_action( 'performance_monitor_repeating_task', __NAMESPACE__ . '\performance_monitor_do_task', 10, 2 );
...
abstract class Task {
  ...
  public $useCount = 0;
  public $hookName = 'performance_monitor_task';
  public $siteId;
  public $timeout;
  ...
  public static function restorePersisted( $taskName ) {
    $transientName = PERFORMANCE_MONITOR_PREFIX . 'task' . $taskName;
  ...
  protected function persist() {
    ...
    $transientName = PERFORMANCE_MONITOR_PREFIX . 'task' . $jobName;
    set_transient( $transientName, $this, PERFORMANCE_MONITOR_SHORT_LIFETIME );
  ...
  public function setStatus( $status, $available = null, $active = null, $fraction = null ) {
    ...
    $jobResultName = PERFORMANCE_MONITOR_PREFIX . 'result' . self::toSnake( $this->taskName );
    set_transient( $jobResultName, $status, PERFORMANCE_MONITOR_LONG_LIFETIME );
  ...
  protected function clearPersisted() {
    ...
    $transientName = PERFORMANCE_MONITOR_PREFIX . 'task' . $jobStatusName;
